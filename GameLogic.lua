...
GameLogic.CT_THREE_TAKE_ONE2  = 9    -- 三带两单--新加
GameLogic.CT_FOUR_TAKE_ONE    = 10   -- 四带两单
GameLogic.CT_FOUR_TAKE_TWO    = 11   -- 四带两对
GameLogic.CT_FOUR_TAKE_THREE  = 12   -- 四带三----新加
GameLogic.CT_BOMB_CARD        = 13   -- 炸弹类型
GameLogic.CT_MISSILE_CARD     = 14   -- 火箭类型
GameLogic.CT_THREE_LINE_TAKE_XN    = 15   -- 三连带X*N

--保存游戏规则
function GameLogic:saveGeiZeBiaoShi(pGuiZe)
    GLCGuiZeFlag.sanbudai = pGuiZe.sanbudai
    GLCGuiZeFlag.sandaiyi = pGuiZe.sandaiyi
    GLCGuiZeFlag.sandaier = pGuiZe.sandaier
    GLCGuiZeFlag.sidaisan = pGuiZe.sidaisan
    GLCGuiZeFlag.zuihouyishou = pGuiZe.zuihouyishou
    GLCGuiZeFlag.sanAshizhadan = pGuiZe.sanAshizhadan
    dump(GLCGuiZeFlag, "###########saveGeiZeBiaoShi")
end
...
function GameLogic:CompareCard(cbFirstCard,cbFirstCount,cbNextCard,cbNextCount)
    ...
    if cbFirstType ~= cbNextType or cbFirstCount ~= cbNextCount then
        if (cbFirstType == 9 and cbNextType == 8) or (cbFirstType == 8 and cbNextType == 9) then 
            print("三带2单 和 三带2对")
        elseif (cbFirstType == cbNextType) and (cbFirstCount == 4 and cbNextCount == 3) and 
        ((cbNextCard[1] == 1 or cbNextCard[1] == 17 or cbNextCard[1] == 33 or cbNextCard[1] == 49) or
        (cbNextCard[2] == 1 or cbNextCard[2] == 17 or cbNextCard[2] == 33 or cbNextCard[2] == 49) or
        (cbNextCard[3] == 1 or cbNextCard[3] == 17 or cbNextCard[3] == 33 or cbNextCard[3] == 49)) then
            print("三个A是炸弹")
        else
            return false
        end
    ...
    elseif (cbNextType == GameLogic.CT_THREE_TAKE_ONE) or (cbNextType == GameLogic.CT_THREE_TAKE_TWO) or (cbNextType == GameLogic.CT_THREE_TAKE_ONE2) then
    ...
    elseif (cbNextType == GameLogic.CT_FOUR_TAKE_ONE) or (cbNextType == GameLogic.CT_FOUR_TAKE_TWO) then
        ...
        return cbNextLogicValue > cbFirstLogicValue
    elseif (cbNextType == GameLogic.CT_FOUR_TAKE_THREE) then
        local nextResult = GameLogic:AnalysebCardData(cbNextCard, cbNextCount)
        local firstResult = GameLogic:AnalysebCardData(cbFirstCard, cbFirstCount)
        local cbNextLogicValue = GameLogic:GetCardLogicValue(nextResult[2][4][1])
        local cbFirstLogicValue = GameLogic:GetCardLogicValue(firstResult[2][4][1])
        return cbNextLogicValue > cbFirstLogicValue
    elseif (cbNextType == GameLogic.CT_THREE_LINE_TAKE_XN) then--相当于飞机
        local nextResult = GameLogic:AnalysebCardData(cbNextCard, cbNextCount)
        local firstResult = GameLogic:AnalysebCardData(cbFirstCard, cbFirstCount)
        local cbNextLogicValue = GameLogic:GetCardLogicValue(nextResult[2][3][1])
        local cbFirstLogicValue = GameLogic:GetCardLogicValue(firstResult[2][3][1])
...
function GameLogic:GetCardType(cbCardData, cbCardCount)
    ...
    if tagAnalyseResult[1][4] > 0 then
        ...
        if tagAnalyseResult[1][4] == 1 and cbCardCount == 7 and GLCGuiZeFlag.sidaisan == 1 then
            return GameLogic.CT_FOUR_TAKE_THREE
        end
        
        if  tagAnalyseResult[1][3] >= 2  and tagAnalyseResult[1][4] >= 1 then
    ...
    if tagAnalyseResult[1][3] > 0 then
        ...
        elseif cbCardCount == 3 then
            --3张A是炸弹  ：4张A == 1 17 33 49
            if GLCGuiZeFlag.sanAshizhadan == 1 then
                if cbCardData[1] == 1 or cbCardData[1] == 17 or cbCardData[1] == 33 or cbCardData[1] == 49 then
                    if cbCardData[2] == 1 or cbCardData[2] == 17 or cbCardData[2] == 33 or cbCardData[2] == 49 then
                        if cbCardData[3] == 1 or cbCardData[3] == 17 or cbCardData[3] == 33 or cbCardData[3] == 49 then
                            return GameLogic.CT_BOMB_CARD
                        end
                    end
                end
        ...
        if tagAnalyseResult[1][3]*3 == cbCardCount and GLCGuiZeFlag.sanbudai == 1 then
            --判断是否连续
            local cbCard = tagAnalyseResult[2][3][1]
            local cbFirstLogicValue = GameLogic:GetCardLogicValue(cbCard)
            for i=2,tagAnalyseResult[1][3] do
                ...
                if cbFirstLogicValue ~= cbNextLogicValue+i-1 then
                    ...
                    local tmpnum = 1
                    local isLianXu = false
                    if isLianXu == false then
                        local geshu = tagAnalyseResult[1][3]
                        for i=1,geshu-1 do
                            for j=i,geshu-1 do
                                local tempnumber1 = GameLogic:GetCardLogicValue(tagAnalyseResult[2][3][i*3]) - 1
                                local tempnumber2 = GameLogic:GetCardLogicValue(tagAnalyseResult[2][3][(j+1)*3])
                    ...
                    if isLianXu then
                        local danpaiCount = tmpnum*3 + tmpnum
                        local shuangpaiCount = tmpnum*3 + tmpnum*2
                        if cbCardCount == danpaiCount  or cbCardCount == shuangpaiCount then
                              return GameLogic.CT_THREE_LINE_TAKE_XN
                        end
                    end
        ...
        if tagAnalyseResult[1][3] ==1 and tagAnalyseResult[1][3]*4 == cbCardCount and GLCGuiZeFlag.sandaiyi == 1 then
            --最后一手没开启
            if GLCGuiZeFlag.zuihouyishou == 0 then
                return GameLogic.CT_THREE_TAKE_ONE
            end

            --最后一手开启了 但手牌数只有4张
            if GLCGuiZeFlag.zuihouyishou == 1 and myHandCardCount == 4 then
                return GameLogic.CT_THREE_TAKE_ONE
            end
        end
        if tagAnalyseResult[1][3] ==1 and tagAnalyseResult[1][3]*5 == cbCardCount  and tagAnalyseResult[1][2] == tagAnalyseResult[1][3] and GLCGuiZeFlag.sandaier == 1 then
           return GameLogic.CT_THREE_TAKE_TWO
        end
        if tagAnalyseResult[1][3] ==1 and tagAnalyseResult[1][3]*5 == cbCardCount  and tagAnalyseResult[1][1] == 2  and  tagAnalyseResult[1][3] == 1 and GLCGuiZeFlag.sandaier == 1 then
           return GameLogic.CT_THREE_TAKE_ONE2
        end

        if  tagAnalyseResult[1][3] >= 2  then
            if tagAnalyseResult[1][4] == 0 then--没有炸弹组合
                --判断是否连续
                local isLianXu = true
                local cbCard = tagAnalyseResult[2][3][1]
                local cbFirstLogicValue = GameLogic:GetCardLogicValue(cbCard)
                ...
                if isLianXu == false then
                    ...
                    tmpnum = tagAnalyseResult[1][3]--3个一样的个数
                end
                ...
                local tmpnum = tagAnalyseResult[1][3]--3个一样的个数
                local tmpnum4 = tagAnalyseResult[1][4]--4个一样的个数
                local tmpnum1 = tagAnalyseResult[1][1]--1个一样的个数
                local tmpnum2 = tagAnalyseResult[1][2]--2个一样的个数
                local cbCard3ext1 = tagAnalyseResult[2][3][1]--第1张牌
                local cbCard3ext2 = tagAnalyseResult[2][3][tmpnum*3]--第6张牌
                local cbCard4 = tagAnalyseResult[2][4][1]
                local paizu3_1 = GameLogic:GetCardLogicValue(cbCard3ext1)
                local paizu3_2 = GameLogic:GetCardLogicValue(cbCard3ext2)
                local paizu4 = GameLogic:GetCardLogicValue(cbCard4)
                local danpaiCount = (tmpnum+1)*3 + (tmpnum+1)
                local shuangpaiCount = (tmpnum+1)*3 + (tmpnum+1)*2
                local danpaiNum = tmpnum*3 + tmpnum
                local shuangpaiNum = tmpnum*3 + tmpnum*2
                local jisuanCount = tmpnum1 + tmpnum4*4 + tmpnum*3

                local TempPaizu4_1 = paizu4-1
                local TempPaizu4_2 = paizu4+1
                --999888+7777情况
                if isLianXu == true and tmpnum == 2 and tmpnum4 == 1 and (cbCardCount == danpaiNum  or cbCardCount == shuangpaiNum)then
                    return GameLogic.CT_THREE_LINE_TAKE_XN
                end
                --9998887777+65情况
                if TempPaizu4_2 == paizu3_2 and (cbCardCount == danpaiCount  or cbCardCount == shuangpaiCount)then
                    return GameLogic.CT_THREE_LINE_TAKE_XN
                end
                --9+999888777+65情况
                if TempPaizu4_1 == paizu3_1 and (cbCardCount == danpaiCount  or cbCardCount == shuangpaiCount)then
                    return GameLogic.CT_THREE_LINE_TAKE_XN
                end

                --999+8888777+65情况
                if TempPaizu4_2 == paizu3_1 and TempPaizu4_1 == paizu3_2 and (cbCardCount == danpaiCount  or cbCardCount == shuangpaiCount)then
                    return GameLogic.CT_THREE_LINE_TAKE_XN
                end

                --999+8888777+333365情况
                if isLianXu ==true and tmpnum4 == 1 and tmpnum1 == 2  and cbCardCount == jisuanCount then
                    return GameLogic.CT_THREE_LINE_TAKE_XN
                end
            end
        end
...
function GameLogic:SearchOutCard(cbHandCardData,cbHandCardCount,cbTurnCardData,cbTurnCardCount) 
    ...
    myHandCardCount = cbHandCardCount--临时保存手牌数
    --出牌分析
    ...
    if cbTurnOutType == GameLogic.CT_ERROR then --错误类型
        ...
        --print("四带三")
        --四带三
        local tagSearchCardResult4_3 = GameLogic:SearchTakeCardTypeSiDaiSan(cbCardData, cbCardCount, 0, 4, 3)
        cbTmpCount = tagSearchCardResult4_3[1]
        if cbTmpCount > 0 then
            for i=1,cbTmpCount do
                cbResultCardCount[cbResultCount] = tagSearchCardResult4_3[2][i]
                cbResultCard[cbResultCount] = {}
                cbResultCard[cbResultCount] = tagSearchCardResult4_3[3][i]
                cbResultCount = cbResultCount + 1
            end
            tagSearchCardResult[2] = cbResultCardCount
            tagSearchCardResult[3] = cbResultCard
    ...
    elseif cbTurnOutType == GameLogic.CT_THREE_TAKE_ONE or cbTurnOutType == GameLogic.CT_THREE_TAKE_TWO or cbTurnOutType == GameLogic.CT_THREE_TAKE_ONE2 then
        --三带一单、三带一对
    ...
    elseif cbTurnOutType == GameLogic.CT_FOUR_TAKE_ONE or cbTurnOutType == GameLogic.CT_FOUR_TAKE_TWO then
        ...
        tagSearchCardResult[3] = cbResultCard
        tagSearchCardResult[1] = cbResultCount - 1
    elseif cbTurnOutType == GameLogic.CT_FOUR_TAKE_THREE then
        --四带三
        local cbTakeCardCount = 3
        local cbTmpTurnCard = GameLogic:SortOutCardList(cbTurnCardData,cbTurnCardCount)
        local tagSearchCardResult51 = GameLogic:SearchTakeCardTypeSiDaiSan(cbCardData, cbCardCount, cbTmpTurnCard[1], 4, cbTakeCardCount)
        cbResultCount = tagSearchCardResult51[1]
        cbResultCount = cbResultCount + 1
        cbResultCardCount = tagSearchCardResult51[2]
        tagSearchCardResult[2] = cbResultCardCount
        ...
        tagSearchCardResult[3] = cbResultCard
        tagSearchCardResult[1] = cbResultCount - 1
    elseif cbTurnOutType == GameLogic.CT_THREE_LINE_TAKE_XN then--新牌型 相当于飞机
         --飞机
        --print("飞机")


        local tagSearchCardResult7 = GameLogic:SearchThreeTwoLine_XN(cbCardData, cbCardCount,cbTurnCardCount)
        cbTmpCount = tagSearchCardResult7[1]
        
        local lianpai1 = GameLogic:SearchLineCardType(cbCardData, cbCardCount, 0, 3, 0)
        local lianpai2 = GameLogic:SearchLineCardType(cbTurnCardData, cbTurnCardCount, 0, 3, 0)
        if lianpai1[1] > 0 and lianpai2[1] > 0 and (GameLogic:GetCardValue(lianpai1[3][1][1]) < GameLogic:GetCardValue(lianpai2[3][1][1]) or lianpai1[2][1] ~= lianpai2[2][1]) then
            cbTmpCount = 0
        end

        if tagSearchCardResult7[1] == 1 and tagSearchCardResult7[2][1] ~= cbTurnCardCount then--别人打出飞机， 自己找牌找到飞机，但是个数不一样就不加入提示了
    ...
    if (cbCardCount >= 4 and cbTurnOutType ~= GameLogic.CT_MISSILE_CARD) then
        ...
        if tagSearchCardResult[1] == 0 and GLCGuiZeFlag.sanAshizhadan == 1 then
            if cbCardData[1] == 1 or cbCardData[1] == 17 or cbCardData[1] == 33 or cbCardData[1] == 49 then
                if cbCardData[2] == 1 or cbCardData[2] == 17 or cbCardData[2] == 33 or cbCardData[2] == 49 then
                    if cbCardData[3] == 1 or cbCardData[3] == 17 or cbCardData[3] == 33 or cbCardData[3] == 49 then
                        tagSearchCardResult[1] = 1
                        tagSearchCardResult[2][1] = 3
                        cbResultCard[1] = {cbCardData[1],cbCardData[2],cbCardData[3]}
                        tagSearchCardResult[3] = cbResultCard
                    end
                end
            end
...
function GameLogic:SearchTakeCardType(cbHandCardData, cbHandCardCount, cbReferCard, cbSameCount, cbTakeCardCount)
    ...
    local cbResultCard = {}
    --临时保存手牌数
    myHandCardCount = cbHandCardCount
    --搜索结果
    local tagSearchCardResult = {cbResultCount-1,cbResultCardCount,cbResultCard}
    ...
    if cbSameCardResultCount > 0 then
        ...
        for i=1,cbSameCardResultCount do
            ...
            while j <= 4 do
                local k = 1
                if cbTakeCardCount == 2 and tagAnalyseResult[1][2] == 0 then--如果没有对牌查看单牌
                    while k <= tagAnalyseResult[1][1]  do
                        --从小到大
                        local cbIndex = (tagAnalyseResult[1][1]+1-k)
                        if GameLogic:GetCardLogicValue(sameCardResult[3][i][1]) ~= GameLogic:GetCardLogicValue(tagAnalyseResult[2][1][cbIndex]) then
                            --复制带牌
                            local cbCount = sameCardResult[2][i]
                            for l=1,cbTakeCardCount do
                                sameCardResult[3][i][cbCount+l] = tagAnalyseResult[2][1][cbIndex+1-l]
                            end
...
--带牌四带三
function GameLogic:SearchTakeCardTypeSiDaiSan(cbHandCardData, cbHandCardCount, cbReferCard, cbSameCount, cbTakeCardCount)
    --结果数目
    local cbResultCount = 1
    --扑克数目
    local cbResultCardCount = {}
    ...
    local paiTab4 = {}--4张牌
    local paiZongShu = {}--牌总数
    if cbSameCardResultCount > 0 then
        --分析结构
        local tagAnalyseResult = GameLogic:AnalysebCardData(cbCardData, cbCardCount)
        --单牌总数量
        local danpaiNum = tagAnalyseResult[1][1]
        --对牌总数量
        local duipaiNum = tagAnalyseResult[1][2]
        --连队总数量
        local lianpaiNum = tagAnalyseResult[1][3]
        --炸弹总数量
        local zhadanNum = tagAnalyseResult[1][4]
       
        --判断3连是否有
        local loopNumber1 = 1--总组合个数
        local loopNumber2 = 1--牌组循环个数

        --三张牌如果有
        if  lianpaiNum > 0 then
            for i=1,lianpaiNum do
                --获取4张牌
                for j=1,zhadanNum do
                    paiTab4[loopNumber1] = clone(sameCardResult[3][j])--4张牌
                    paiZongShu[loopNumber1] = clone(sameCardResult[2][j])--牌总数

                    --带3张牌
                    for k=1,3 do
                        paiTab4[loopNumber1][4+k] = tagAnalyseResult[2][3][loopNumber2]
                        loopNumber2 = loopNumber2 + 1 -- 别的地方用记得 初始化
                    end
                    paiZongShu[loopNumber1] = paiZongShu[loopNumber1] + 3--手牌数加+3
                    loopNumber1 = loopNumber1 + 1--总组合数，只增加 不减
                end
            end
        end

        --对牌+单牌凑3张
        if danpaiNum > 0 and duipaiNum > 0 then
            for i=1,duipaiNum do
                --获取4张牌
                for j=1,zhadanNum do
                    loopNumber2 = i
                    paiTab4[loopNumber1] = clone(sameCardResult[3][j])--4张牌
                    ...
                    for k=1,3 do
                        if k == 3 then
                            paiTab4[loopNumber1][4+k] = tagAnalyseResult[2][1][1]
                        else
                            paiTab4[loopNumber1][4+k] = tagAnalyseResult[2][2][loopNumber2]
                        end
        ...
        if danpaiNum > 3 then
            for i=1,danpaiNum-2 do
                ...
                for j=1,zhadanNum do
                    ...
                    --带3张单牌
                    for k=1,3 do
                        paiTab4[loopNumber1][4+k] = tagAnalyseResult[2][1][loopNumber2]
                        loopNumber2 = loopNumber2 + 1 -- 别的地方用记得 初始化
                    end
                    paiZongShu[loopNumber1] = paiZongShu[loopNumber1] + 3--手牌数加+3
                    loopNumber1 = loopNumber1 + 1--总组合数，只增加 不减
        ...
        --保存结果
        tagSearchCardResult[1] = loopNumber1-1
        tagSearchCardResult[2] = paiZongShu
        tagSearchCardResult[3] = paiTab4
    end
...
--飞机搜索
function GameLogic:SearchThreeTwoLine_XN(cbHandCardData, cbHandCardCount,cbAllCardCount)
    --print("飞机搜索")
    --结果数目
    local cbSearchCount = 0
    --扑克数目
    ...
    if cbTmpResultCount > 0 then
        ...
        while i <= cbTmpResultCount do
            ...
            if flag == true then
                ...
                --组合飞机
                local cbNeedCount = cbAllCardCount - tmpSearchResult[2][i]
                local cbResultCount = tmpSingleWing[1]+1
                tmpSingleWing[3][cbResultCount] = tmpSearchResult[3][i]
                for j=1,cbNeedCount do
                    tmpSingleWing[3][cbResultCount][tmpSearchResult[2][i]+j] = cbTmpCardData[cbTmpCardCount-cbNeedCount+j]
                ...
                local flag2 = true
                --不够带翅膀
                if cbTmpCardCount < cbNeedCount then
                    local cbNeedDelCount = 3
                    while (cbTmpCardCount + cbNeedDelCount - tmpSearchResult[2][i]) < (tmpSearchResult[2][i]-cbNeedDelCount)/3*2 do
                        cbNeedDelCount = cbNeedDelCount + 3
                    end
...
function GameLogic:SortOutCardList(cbCardData,cbCardCount)
    ...
    if cbCardType == GameLogic.CT_THREE_TAKE_ONE or cbCardType == GameLogic.CT_THREE_TAKE_TWO or cbCardType == GameLogic.CT_THREE_TAKE_ONE2 then
        --分析牌
        local AnalyseResult = {}
    ...
    elseif cbCardType == GameLogic.CT_FOUR_TAKE_THREE then
        --分析牌
        local AnalyseResult = {}
        AnalyseResult = GameLogic:AnalysebCardData(cbCardData,cbCardCount)
